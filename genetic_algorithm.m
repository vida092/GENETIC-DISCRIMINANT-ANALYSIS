function [best_individual, best_fitness, plot_line] = genetic_algorithm(csv_file, num_individuals, num_generations, crossover_prob)
    ...
    % Etiquetas únicas y colores para hasta 3 clases
    classes = unique(y);
    colors = {'r', 'g', 'b'};
    class_labels = arrayfun(@(c) sprintf('Class %d', c), classes, 'UniformOutput', false);

    % Figura de proyección: subplot con 2D y 3D
    figure;
    ...
    for i = 1:length(classes)
        idx = y == classes(i);
        markers = {'o', 's', '^'}; % marcador por clase
        scatter(X_projected(idx,1), X_projected(idx,2), 36, ...
                'Marker', markers{i}, ...
    ...
    for i = 1:length(classes)
        idx = y == classes(i);
        scatter3(X_projected(idx,1), X_projected(idx,2), X_projected(idx,3), 36, ...
         'Marker', markers{i}, ...
         'MarkerEdgeColor', colors{i}, ...
         'MarkerFaceColor', 'none', ...
         'LineWidth', 1.2);
    ...
    figure;
    plot(1:num_generations, plot_line, '-', 'LineWidth', 1.5);
    xlabel('Generatopm');
    ylabel('Best Fitness');
